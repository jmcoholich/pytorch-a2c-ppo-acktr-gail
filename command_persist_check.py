...
def main():
    ...
    while True: # Sample an episode
        ...
        if counter == 0:
            action = torch.from_numpy(env.action_space.high).unsqueeze(0)
            obs, reward, done, info = env.step(action)
        else:
            p.stepSimulation()
            env.venv.venv.envs[0]._update_state()
            done, info = env.venv.venv.envs[0]._is_state_terminal()
            reward = env.venv.venv.envs[0]._reward_function()
            obs = env.venv.venv.envs[0].state
            done = False
        # action = np.clip(action, env.action_space.low, env.action_space.high)

        # action = torch.from_numpy(np.sin(np.ones(12) * counter * 2 * np.pi/50)).unsqueeze(0)
        episode_rew += reward
        ...
        if counter % (240 * VIDEO_SPEED/ FPS) == 0 : # TODO change back
            img_array = add_frame(render_func, img_array)
        counter += 1
        if counter == 10:
            break
